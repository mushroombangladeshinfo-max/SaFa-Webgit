import"./modulepreload-polyfill-B5Qt9EMX.js";import"./supabase-DFvBSl1A.js";import"./main-DHnC8REm.js";import"./auth-nav-rpXGp03P.js";import{c as f}from"./index-B-jIxwbw.js";import"./admin-auth-4ZiUUGs_.js";const b=f("https://uiwmerejtrdrykqpumdu.supabase.co","eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVpd21lcmVqdHJkcnlrcXB1bWR1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU3NzQ1MjIsImV4cCI6MjA5MTM1MDUyMn0.bF1GPeGFhAphNFG-E6MfZCrZihT3iTeCFIDi6g3w0n0"),w=[{key:"pending",label:`অর্ডার
...
গেছে`,icon:B()}],m=["pending","confirmed","processing","shipped","delivered"];function y(){return`<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
`,"<br/>")}</div>
        ...
        </div>`).join(""):'<div class="item-row"><span class="item-name" style="color:var(--muted)">পণ্যের বিবরণ পাওয়া যায়নি</span></div>',g=s.reduce((t,a)=>t+(a.price||0)*(a.qty||a.quantity||1),0),d=e.delivery_charge||0,k=encodeURIComponent(`হ্যালো, আমার অর্ডার নম্বর ${e.order_number} সম্পর্কে জানতে চাই।`);return`
  ...
  `}window.trackOrder=async function(){E();const e=document.getElementById("inp-order").value.trim(),n=document.getElementById("inp-phone").value.trim();if(!e){l("অর্ডার নম্বর দিন।");return}if(!n){l("ফোন নম্বর দিন।");return}u(!0),document.getElementById("result-card").classList.remove("show");try{const{data:s,error:o}=await b.rpc("track_order",{p_order_number:e,p_phone:n});if(o)throw o;if(!s){l("অর্ডার পাওয়া যায়নি। অর্ডার নম্বর ও ফোন নম্বর মিলিয়ে দেখুন।");return}const r=document.getElementById("result-card");r.innerHTML=x(s),r.classList.add("show"),r.scrollIntoView({behavior:"smooth",block:"start"})}catch(s){l("সংযোগে সমস্যা হয়েছে। একটু পরে আবার চেষ্টা করুন।"),console.error(s)}finally{u(!1)}};document.addEventListener("keydown",e=>{e.key==="Enter"&&(document.activeElement===document.getElementById("inp-order")||document.activeElement===document.getElementById("inp-phone"))&&window.trackOrder()});const i=new URLSearchParams(location.search);i.get("order")&&(document.getElementById("inp-order").value=i.get("order"));i.get("phone")&&(document.getElementById("inp-phone").value=i.get("phone"));i.get("order")&&i.get("phone")&&window.trackOrder();
