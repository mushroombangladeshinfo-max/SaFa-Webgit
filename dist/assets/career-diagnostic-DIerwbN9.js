...
`):"(none supplied)"}function f({cvText:i,initialIntent:e}){return[{role:"system",content:l()},{role:"user",content:`INITIAL USER INTENT (may be incomplete):
${o(e)}
...
TASK:
Perform the first critical CV + career-intent diagnostic. Build a conservative profile draft and strategy draft. If one material ambiguity remains, ask the single highest-impact clarification question. Otherwise mark it ready for user confirmation.`}]}function h({initialIntent:i,previousResult:e,answers:n,questionLimitReached:t}){const a=n[n.length-1];return[{role:"system",content:u()},{role:"user",content:`INITIAL USER INTENT:
${o(i)}
...
TASK:
Update the diagnostic, profile draft, and strategy draft using the verified answer, then either ask ONE new high-impact question or mark ready for confirmation.`}]}function p({initialIntent:i,previousResult:e,answers:n,correction:t}){return[{role:"system",content:g()},{role:"user",content:`INITIAL USER INTENT:
${o(i)}
