import{f as H}from"./admin-auth-4ZiUUGs_.js";const E=[{key:"home",href:"home.html",label:"Overview",icon:"🏠"},{key:"orders",href:"orders.html",label:"Orders",icon:"📦"},{key:"customers",href:"customers.html",label:"Customers",icon:"👤"},{key:"products",href:"products.html",label:"Products",icon:"🍄"},{key:"expenses",href:"expenses.html",label:"Expenses",icon:"💰"},{key:"pipeline",href:"pipeline.html",label:"B2B",icon:"🤝"},{key:"insights",href:"insights.html",label:"Insights",icon:"📊"}],$=[{href:"quick-log.html",label:"⚡ Quick Log"},{href:"harvest-log.html",label:"🍄 Harvest Log"},{href:"spawn-lab.html",label:"🧫 Spawn Lab"},{href:"farm-analytics.html",label:"📈 Farm Analytics"},{href:"dashboard.html",label:"📡 IoT Dashboard"}],I=[{group:"Start Here",links:[{href:"job-dashboard.html",label:"🎯 Dashboard"},{href:"job-settings.html",label:"⚙ Career Profile"}]},{group:"Search & Track",links:[{href:"job-opportunities.html",label:"💼 Opportunities"},{href:"job-discover.html",label:"🔎 Discover Jobs"}]},{group:"Engage",links:[{href:"job-contacts.html",label:"🗂 Contacts"},{href:"job-interviews.html",label:"🎤 Interviews"},{href:"job-activities.html",label:"≡ Activities"},{href:"job-offers.html",label:"◆ Offers"}]},{group:"Review",links:[{href:"job-analytics.html",label:"▥ Analytics"},{href:"job-skills.html",label:"✦ Skills Intelligence"}]},{group:"Tools",links:[{href:"job-resumes.html",label:"📄 CV Studio"},{href:"job-quick-log.html",label:"📝 Quick Log"},{href:"job-copilot.html",label:"✦ AI Copilot"}]}],M=[{href:"home.html",label:"Overview",icon:"🏠",hint:"Dashboard & KPIs",keywords:"home dashboard summary kpi"},{href:"orders.html",label:"Orders",icon:"📦",hint:"Manage customer orders",keywords:"sales orders shipping fulfillment"},{href:"customers.html",label:"Customers",icon:"👤",hint:"Customer directory",keywords:"customers users contacts crm"},{href:"products.html",label:"Products",icon:"🍄",hint:"Catalog & inventory",keywords:"products catalog inventory stock"},{href:"expenses.html",label:"Expenses",icon:"💰",hint:"Costs & P&L",keywords:"expenses costs finance pnl profit loss"},{href:"pipeline.html",label:"B2B",icon:"🤝",hint:"Wholesale pipeline",keywords:"b2b pipeline wholesale deals leads"},{href:"insights.html",label:"Insights",icon:"📊",hint:"AI analyst & trends",keywords:"insights analytics ai reports trends"},{href:"quick-log.html",label:"Quick Log",icon:"⚡",hint:"Mobile daily farm log",keywords:"quick log daily farm mobile harvest"},{href:"harvest-log.html",label:"Harvest Log",icon:"🍄",hint:"Every harvest entry, filterable",keywords:"harvest log entries batches rooms pivot table"},{href:"spawn-lab.html",label:"Spawn Lab",icon:"🧫",hint:"In-house culture & grain spawn production",keywords:"spawn lab culture agar petri liquid grain inoculation production"},{href:"farm-analytics.html",label:"Farm Analytics",icon:"📈",hint:"Growth & yield charts",keywords:"farm analytics yield growth charts"},{href:"dashboard.html",label:"IoT Dashboard",icon:"📡",hint:"Simulated sensor preview",keywords:"iot dashboard sensors demo"},{href:"job-dashboard.html",label:"Job Dashboard",icon:"🎯",hint:"Job search command center",keywords:"job search dashboard priorities pipeline funnel"},{href:"job-opportunities.html",label:"Opportunities",icon:"💼",hint:"Job applications tracker",keywords:"job opportunities applications kanban ats fit score"},{href:"job-contacts.html",label:"Job Contacts",icon:"🗂",hint:"Networking CRM",keywords:"job contacts networking recruiter referral"},{href:"job-interviews.html",label:"Interviews",icon:"🎤",hint:"Interview prep & tracking",keywords:"job interviews prep questions star"},{href:"job-activities.html",label:"Job Activities",icon:"≡",hint:"Every applications/message/call, chronologically",keywords:"job activities log timeline history networking"},{href:"job-offers.html",label:"Job Offers",icon:"◆",hint:"Track and compare offers",keywords:"job offers salary compare negotiation"},{href:"job-skills.html",label:"Skills Intelligence",icon:"✦",hint:"Skill gaps across your job search",keywords:"job skills intelligence gaps proficiency"},{href:"job-analytics.html",label:"Job Analytics",icon:"▥",hint:"Funnel, conversion, CV performance",keywords:"job analytics funnel conversion rejection cv performance"},{href:"job-copilot.html",label:"AI Copilot",icon:"✦",hint:"Job search AI assistant",keywords:"job ai copilot assistant chat"},{href:"job-settings.html",label:"Career Profile",icon:"⚙",hint:"Background used by AI",keywords:"job career profile settings resume cv"},{href:"job-resumes.html",label:"CV Studio",icon:"📄",hint:"Generate & tailor CVs",keywords:"job cv studio resume tailor ats base"},{href:"job-quick-log.html",label:"Quick Log",icon:"📝",hint:"Fast mobile-friendly update logging",keywords:"job quick log update mobile fast"},{href:"job-discover.html",label:"Discover Jobs",icon:"🔎",hint:"Search live BA job board postings",keywords:"job discover search bundesagentur arbeit sourcing"}];function O(){if(document.getElementById("admin-nav-styles"))return;const o=document.createElement("style");o.id="admin-nav-styles",o.textContent=`
  ...
  `,document.head.appendChild(o)}function q({page:o,supabase:z,email:u,lastLogin:x}){O();const b=H(u),C=b.includes("&")?"F·S":b.charAt(0);document.querySelector("header.an")?.remove(),document.getElementById("an-mobile-drawer")?.remove(),document.getElementById("an-palette-overlay")?.remove();const B=E.map(e=>`
    <a href="${e.href}" class="an-link${o===e.key?" active":""}" aria-current="${o===e.key?"page":"false"}">
      <span class="an-link-icon">${e.icon}</span>${e.label}
    </a>`).join(""),A=$.map(e=>`<a href="${e.href}">${e.label}</a>`).join(""),D=I.map(e=>`
    <div class="an-drop-group-label">${e.group}</div>
    ${e.links.map(a=>`<a href="${a.href}">${a.label}</a>`).join("")}
  `).join(""),J=`
    ...
    <div class="an-mobile-drawer" id="an-mobile-drawer" role="navigation" aria-label="Mobile navigation">
      ...
      ${$.map(e=>`<a href="${e.href}" class="an-link" style="letter-spacing:0;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:400;text-transform:none;">${e.label}</a>`).join("")}
      <div style="height:1px;background:rgba(255,255,255,.06);margin:8px 0;"></div>
      ...
      ${I.map(e=>`
        <div style="font-family:'Syne',sans-serif;font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(245,239,230,.32);padding:6px 16px 2px;">${e.group}</div>
        ${e.links.map(a=>`<a href="${a.href}" class="an-link" style="letter-spacing:0;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:400;text-transform:none;">${a.label}</a>`).join("")}
      `).join("")}
      <div style="height:1px;background:rgba(255,255,255,.06);margin:8px 0;"></div>
    ...
    <div class="an-palette-overlay" id="an-palette-overlay" role="dialog" aria-modal="true" aria-label="Search admin pages">
    ...
    </div>`,y=document.createElement("div");y.innerHTML=J,document.body.prepend(...y.children);const r=document.getElementById("an-farm-btn"),g=document.getElementById("an-farm-menu");r?.addEventListener("click",e=>{e.stopPropagation();const a=g.classList.toggle("open");r.classList.toggle("open",a),r.setAttribute("aria-expanded",a),h?.classList.remove("open"),i?.classList.remove("open")});const i=document.getElementById("an-jobs-btn"),h=document.getElementById("an-jobs-menu");i?.addEventListener("click",e=>{e.stopPropagation();const a=h.classList.toggle("open");i.classList.toggle("open",a),i.setAttribute("aria-expanded",a),g?.classList.remove("open"),r?.classList.remove("open")}),document.addEventListener("click",()=>{g?.classList.remove("open"),r?.classList.remove("open"),r?.setAttribute("aria-expanded","false"),h?.classList.remove("open"),i?.classList.remove("open"),i?.setAttribute("aria-expanded","false")});const v=document.getElementById("an-burger"),T=document.getElementById("an-mobile-drawer");v?.addEventListener("click",()=>{const e=T.classList.toggle("open");v.setAttribute("aria-expanded",e)});async function k(){await z.auth.signOut(),window.location.href="orders.html"}document.getElementById("an-logout-btn")?.addEventListener("click",k),document.getElementById("an-logout-mobile")?.addEventListener("click",k);const s=document.getElementById("an-palette-overlay"),p=document.getElementById("an-palette-input"),f=document.getElementById("an-palette-list");let n=M.filter(e=>e.href!==w()),t=0;function w(){return location.pathname.split("/").pop()||"home.html"}function c(){if(!n.length){f.innerHTML='<div class="an-palette-empty">No pages match that search.</div>';return}f.innerHTML=n.map((e,a)=>`
      ...
      </a>`).join("")}function j(e){const a=e.trim().toLowerCase(),d=M.filter(l=>l.href!==w());n=a?d.filter(l=>l.label.toLowerCase().includes(a)||l.keywords.includes(a)||a.split(/\s+/).every(S=>l.keywords.includes(S)||l.label.toLowerCase().includes(S))):d,t=0,c()}function L(){s.classList.add("open"),p.value="",j(""),setTimeout(()=>p.focus(),0)}function m(){s.classList.remove("open")}document.getElementById("an-search-btn")?.addEventListener("click",L),document.addEventListener("keydown",e=>{(e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="k"?(e.preventDefault(),s.classList.contains("open")?m():L()):e.key==="Escape"&&s.classList.contains("open")&&m()}),s.addEventListener("mousedown",e=>{e.target===s&&m()}),p.addEventListener("input",()=>j(p.value)),p.addEventListener("keydown",e=>{if(e.key==="ArrowDown")e.preventDefault(),n.length&&(t=(t+1)%n.length,c());else if(e.key==="ArrowUp")e.preventDefault(),n.length&&(t=(t-1+n.length)%n.length,c());else if(e.key==="Enter"){e.preventDefault();const a=n[t];a&&(window.location.href=a.href)}}),f.addEventListener("mousemove",e=>{const a=e.target.closest(".an-palette-item");if(!a)return;const d=Number(a.dataset.idx);d!==t&&(t=d,c())})}export{q as m};
